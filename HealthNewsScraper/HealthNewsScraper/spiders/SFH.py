...
class SfhSpider(scrapy.Spider):
    ...
    **scrapy genspider spidername domain**. This Spider crawls and
    ...
    allowed_domains = ['sfhnigeria.org']

    # The URL the spider begins crawling from
    start_urls = ['https://www.sfhnigeria.org/category/news/news-from-the-field/']

    # extra settings for this spider
    custom_settings = {
        "FEEDS": {
            "./HealthNewsScraper/scrapes/sfh.jl": {"format": "jsonlines"}
        },
    }
    ...
    def parse(self, response):
        ...
        next_page_url = response.css("div.pagination-full a.pagination-next::attr(href)").get()
    ...
    def scrape(self, response):
        ...
        for news_card in response.css("div.post-row.item"):
            individual_news_link = news_card.css("h3.post-title a::attr(href)").get()
    ...
    def news_reader(response):
        ...
        news_item_loader.add_css('title', 'h4.classic-title::text')
        news_item_loader.add_css('body', 'div.entry-content p::text')
