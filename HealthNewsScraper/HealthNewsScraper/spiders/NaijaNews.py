...
class NaijanewsSpider(scrapy.Spider):
    ...
    allowed_domains = ['naijanews.com']

    # The URL the spider begins crawling from
    start_urls = ["https://www.naijanews.com/health/"]

    custom_settings = {
        "FEEDS": {
            "./HealthNewsScraper/scrapes/naijanews.jl": {"format": "jsonlines"}
        },
    }
    ...
    def scrape(self, response):
        ...
        for news_card in response.css("li.mvp-blog-story-wrap.left.relative.infinite-post"):
    ...
    def news_reader(response):
        ...
        article = response.css("div#mvp-post-main")
        ...
        news_item_loader.add_css('title', 'h1.mvp-post-title.left.entry-title::text')
        news_item_loader.add_css('body', 'div#mvp-content-main p::text')
        news_item_loader.add_css('date_published', 'time.post-date.updated::text')
