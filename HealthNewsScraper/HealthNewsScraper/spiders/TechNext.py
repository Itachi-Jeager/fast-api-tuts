...
class TechnextSpider(scrapy.Spider):
    ...
    **scrapy genspider spidername domain**. This Spider crawls and
    ...
    allowed_domains = ['technext.ng']

    # The URL the spider begins crawling from
    start_urls = ['https://technext.ng/?s=health']

    # extra settings for this spider
    custom_settings = {
        "FEEDS": {
            "./HealthNewsScraper/scrapes/tech_next.jl": {"format": "jsonlines"}
        },
    }
    ...
    def parse(self, response):
        ...
        next_page = response.css("nav.posts-load-more")
    ...
    def scrape(self, response):
        ...
        for news_card in response.css("article.v-post"):
    ...
    def news_reader(response):
        ...
        article = response.css("article.v-post.single-article")

        # Instantiate an Item loader to populate various fields and person data cleaning
        news_item_loader = ItemLoader(item=TechNextItem(), selector=article)
        ...
        news_item_loader.add_css('title', 'h1.post-head-title::text')
        news_item_loader.add_css('body', 'div.post-inner-content p::text')
